...
let findAuthorsOrderedByLastName = db.any('SELECT last_name FROM authors ORDER BY last_name ASC')
...
// */
let findBookAuthors = db.any('SELECT a.first_name, a.last_name,b.title FROM authors AS a INNER JOIN books AS b ON a.id = b.author_id')
findBookAuthors.then((results)=>{
  assert.deepEqual(results.length, 15)
  assert.deepEqual(results[0].first_name,'John')
  assert.deepEqual(results[results.length - 1].last_name,'Geisel')
})
...
//
// */
let authorIdWithTwoBooks = db.any('SELECT * FROM (SELECT author_id FROM books GROUP BY author_id HAVING COUNT(*) > 1) sub')

authorIdWithTwoBooks.then( (results) => {
    assert.deepEqual(results.length, 2)
    assert.deepEqual(results[0].author_id ,1809)
})
//
// /* --------End of Exercise 5---------------- */
//
...
//b is of books e is for editions
// */
let bookTitlesWithMultipleEditions = db.any('SELECT b.title FROM books b INNER JOIN editions e ON e.book_id = b.id GROUP BY b.id HAVING COUNT(*) > 1')

bookTitlesWithMultipleEditions.then((results)=>{
  console.log(results)
})
// /* --------End of Exercise 6---------------- */
 //
...
authors as auth
ON
title.author_id = auth.author_id
INNER JOIN
daily_inventory
ON
